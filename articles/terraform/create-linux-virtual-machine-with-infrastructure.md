...
마지막 단계에서는 VM을 만들고 생성한 모든 리소스를 사용합니다. 다음 섹션에서는 `myVM`이라는 VM을 만들고 `myNIC`라는 가상 NIC를 연결합니다. 최신 `Ubuntu 18.04-LTS` 이미지를 사용하고, 암호 인증을 사용하지 않도록 설정된 `azureuser`라는 사용자가 만들어집니다.
