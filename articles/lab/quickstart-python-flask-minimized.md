...
- `az` 명령이 인식되지 않는 경우 [초기 환경 설정](#set-up)에서 설명한 대로 Azure CLI가 설치되어 있는지 확인합니다.
