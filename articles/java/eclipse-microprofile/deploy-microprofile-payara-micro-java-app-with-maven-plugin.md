...
이 섹션에서는 PayaraMicro 애플리케이션을 만들고 로컬에서 테스트합니다.
...
이 섹션에서는 Maven이 Azure App Service on Linux에 앱을 배포할 수 있도록 PayaraMicro 프로젝트 *pom.xml* 파일을 구성합니다.
...
4. 런타임을 `TOMCAT 8.5`에서 `java11`로, 배포 파일을 `*.war`에서 `*.jar`로 수정합니다. 그런 다음, `PORT`, `WEBSITES_PORT` 및 `WEBSITES_CONTAINER_START_TIME_LIMIT`의 `<configuration>` 섹션에 `<appSettings>` 섹션을 추가합니다.  
